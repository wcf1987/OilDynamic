...
public class GraphiTools {
	...
	public Map<Integer,Node> NodeMap = null;
	...
	public Graphi calcXY(Graphi g){
		...
		while (edgeBeanIt.hasNext()) {
			...
			Point sourceAction = g.getPoint(String.valueOf(edge.getSourceid()));
			Point targetAction = g.getPoint(String.valueOf(edge.getTargetid()));
			
			String edgeString = "(40)";
			addEdge(sourceAction, targetAction, 40, edgeString);
		}
	...
	protected void applyGraphResults() {
		...
		int i=10;
	    for (;names.hasNext();) {
	    	String namest=names.next();
	    	Integer namei =Integer.valueOf(namest);
	        Node n = NodeMap.get(namei);
	        Point t=graphi.getPoint(namest);
	        if(n!=null){
	        t.setX_location(Double.valueOf(n.y+100));
	        base=base+10;
	        t.setY_location(Double.valueOf(n.x*1.2));
	        }else{
	        	t.setX_location(Double.valueOf(500+base+i));
	  	        i=i+30;
	  	      t.setY_location(Double.valueOf(500+base));
	        }
...
/*		// 建立真正要用的graph的 node edge
	...
	private void addEdgeGef(Point source, Point target, int weight, String edgeString) {
		...
		if (gefSourceNode == null) {
			...
			NodeMap.put(source.getId(), gefSourceNode);
		}
		...
		if (gefTargetNode == null) {
			...
			NodeMap.put(target.getId(), gefTargetNode);
		}
